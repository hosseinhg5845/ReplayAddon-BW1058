...
public class PostgreSQL implements IDatabase {
    ...
    public boolean connect() {
        ...
        hikariConfig.setPoolName("ReplayAddonBW1058PostgreSQLPool");
    ...
    public void init() {
        try (Connection connection = dataSource.getConnection()) {
            String sql = "CREATE TABLE IF NOT EXISTS game_replay (id SERIAL PRIMARY KEY, " +
                    "replay_id VARCHAR(200), replay_data text);";
            try (Statement statement = connection.createStatement()) {
                statement.executeUpdate(sql);
            }
            sql = "CREATE TABLE IF NOT EXISTS proxy_mode_data (id SERIAL PRIMARY KEY, " +
    ...
    public List<GameReplayCache> getGameReplayCaches(String player) {
        ...
        String sql = "SELECT * FROM game_replay WHERE POSITION(? in replay_data)!=0;";
        try (Connection connection = dataSource.getConnection()) {
            try(PreparedStatement statement = connection.prepareStatement(sql)){
