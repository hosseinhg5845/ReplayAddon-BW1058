...
public class SQLite implements IDatabase {
    ...
    public boolean connect() {
        File folder = new File(ReplayPlugin.plugin.getDataFolder() + "/Cache");
        if (!folder.exists()) {
            if (!folder.mkdir()) {
                ReplayPlugin.plugin.getLogger().severe("Could not create /Cache folder!");
            }
        }
        ...
        if (!dataFolder.exists()) {
            try {
                if (!dataFolder.createNewFile()) {
                    ReplayPlugin.plugin.getLogger().severe("Could not create /Cache/cache.db file!");
                }
            } catch (IOException e) {
                e.printStackTrace();
                return false;
        ...
        try {
            ...
            if (e instanceof ClassNotFoundException) {
                ReplayPlugin.plugin.getLogger().severe("Could Not Found SQLite Driver on your system!");
            }
            e.printStackTrace();
            return false;
        }
